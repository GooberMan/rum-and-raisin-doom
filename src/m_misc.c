...
doombool M_FileExists(const char *filename)
{
    FILE *fstream;
...
doombool M_WriteFile(const char *name, const void *source, int length)
{
    FILE *handle;
    int	count;
...
doombool M_StrToInt(const char *str, int *result)
{
...
doombool M_StringCopy(char *dest, const char *src, size_t dest_size)
{
    size_t len;
...
doombool M_StringConcat(char *dest, const char *src, size_t dest_size)
{
    size_t offset;
...
doombool M_StringStartsWith(const char *s, const char *prefix)
{
...
doombool M_StringEndsWith(const char *s, const char *suffix)
{
...
int M_vsnprintf(char *buf, size_t buf_len, const char *s, va_list args)
{
    ...
    if( buf && ( result < 0 || (size_t)result >= buf_len ) )
    {
        buf[buf_len - 1] = '\0';
        result = buf_len - 1;
    }
