...
constexpr uint32_t FNV1aBasis32 = 2166136261u;
constexpr uint64_t FNV1aBasis64 = 14695981039346656037ull;
...
constexpr uint64_t fnv1a64( uint64_t base, uint8_t byte )
{
...
template< typename _ty
		, std::enable_if_t< std::is_integral_v< _ty > && !std::is_same_v< _ty, uint8_t >, bool > = true >
constexpr uint64_t fnv1a64( uint64_t base, const _ty& val )
{
	union
	{
		_ty		base;
	...
	for( uint32_t index = 0; index < sizeof( _ty ); ++index )
	{
		base = fnv1a64( base, mapped.byte[ index ] );
	}

	return base;
}

constexpr uint64_t fnv1a64( uint64_t base, const char* string )
{
	while( string && *string )
	{
		base = fnv1a64( base, (uint8_t)*string );
		++string;
	}
...
struct fnv1a
{
	...
	template< typename _ty >
	static constexpr value_type calc( value_type base, _ty val )
	{
		if constexpr( std::is_same_v< value_type, uint32_t > )
		{
			return fnv1a32( base, val );
		}
		else if constexpr( std::is_same_v< value_type, uint64_t > )
		{
			return fnv1a64( base, val );
		}
	}
};
...
DOOM_C_API typedef uint64_t (*deh_fnv1a64_hash_t)(int32_t version, uint64_t base);
...
DOOM_C_API struct deh_section_s
{
	...
	deh_fnv1a32_hash_t fnv1a32_hash;
	deh_fnv1a64_hash_t fnv1a64_hash;
};
