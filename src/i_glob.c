...
// not all systems.
static doombool IsDirectory(char *dir, struct dirent *de)
{
...
static doombool MatchesGlob(const char *name, const char *glob, int flags)
{
    int n, g;
...
static doombool MatchesAnyGlob(const char *name, glob_t *glob)
{
    int i;
