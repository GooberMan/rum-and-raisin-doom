...
static void DoGenericOnce( line_t* line, mobj_t* activator )
{
	...
	if( !fix.w1_lines_clearing_on_no_result )
	{
		line->action = nullptr;
		line->special = 0;
	}
...
constexpr lineaction_t builtinlineactions[ Actions_BuiltIn_Count ] =
{
	...
	{ &precon::IsPlayer, &DoGenericOnce< Floor >, LT_WalkBoth, LL_None, constants::floorspeeds[ Speed_Slow ], 0, stt_lowestneighborfloor, sd_down, 0, sct_copyboth, scm_numeric, sc_nocrush },
...
static lineaction_t* CreateBoomGeneralisedCeilingAction( line_t* line )
{
	...
	if( direction == sd_down )
	{
		action->param3 = -action->param3;
	}
