...
static doombool pcs_initialized = false;
...
static doombool CachePCSLump(sfxinfo_t *sfxinfo)
{
    int lumplen;
    int headerlen;
...
static doombool IsDisabledSound(sfxinfo_t *sfxinfo)
{
    int i;
...
static doombool I_PCS_SoundIsPlaying(int handle)
{
    if (!pcs_initialized)
    {
        return false;
...
static doombool I_PCS_InitSound(doombool _use_sfx_prefix)
{
