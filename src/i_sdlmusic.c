...
static doombool WriteWrapperTimidityConfig(char *write_path)
{
    char *path;
    FILE *fstream;
...
static doombool I_SDL_InitMusic(void)
{
...
static void I_SDL_PlaySong(void *handle, doombool looping)
{
    int loops;
...
static doombool ConvertMus(byte *musdata, int len, const char *filename)
{
    MEMFILE *instream;
    MEMFILE *outstream;
    void *outbuf;
...
// Is the song playing?
static doombool I_SDL_MusicIsPlaying(void)
{
    if (!music_initialized)
    {
        return false;
